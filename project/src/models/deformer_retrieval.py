...
from src.utils.custom_loss import OnlineTripletLoss, egocentric
...
# from torch_points3d.core.data_transform import RandomDropout,
...
class Deformer_Retrieval(pl.LightningModule):
    def __init__(self, *args, **kwargs):
        ...
        fitting_gap_path = '/vol/vssp/datasets/multiview/3VS/datasets/cache/val_metrics/CD_d.npy'
        self.val_fitting_gap = torch.tensor(np.load(fitting_gap_path)).cuda()
    ...
    def validation_step(self, batch, batch_idx):
        ...
        inputs = torch.cat([source_shape, target_shape])
        features = self.encoder(inputs.transpose(1, 2))
        sketch_feat, shape_feat = torch.split(features, [B, B], dim=0)
    ...
    def validation_epoch_end(self, outputs):
        if not self.trainer.running_sanity_check:
            ...
            for output in outputs:
                sketch_features.append(output['sketch_feat'])
                shape_features.append(output['shape_feat'])
            ...
            acc_at_k_feat_z = compute_acc_at_k(pair_sort)
            ...
            # Top-k fitting_gap
            K = 10
            top_fitting_gap = torch.gather(self.val_fitting_gap, 1, pair_sort[:, :K])
            for k in [1, 5, 10]:
                self.log(f'val/CD_d@{k}', top_fitting_gap[:, :k].mean() * 100, prog_bar=True)
            if 'CD_d' in self.hparams.val_metric:
                k = int(self.hparams.val_metric.split('@')[1])
                self.log('val_loss', top_fitting_gap[:, :k].mean() * 100, prog_bar=True)
            elif 'acc' in self.hparams.val_metric:
                k = self.hparams.val_metric.split('@')[1]
                self.log('val_acc', acc_at_k_feat_z[acc_dict[k]], prog_bar=True)
...
class Deformer_Retrieval_fitting_gap_egocentric(Deformer_Retrieval_fitting_gap):
    ...
    def training_step(self, batch, batch_idx):
        ...
        # Encode [source_sketch, target_shape]
        # inputs = torch.cat([sketch, shape])#torch.Size([12, 1024, 3])
        sketch_feat, matrix = self.encoder(sketch.transpose(1, 2), matrix=True) #torch.Size([8, 512])
        shape_feat = self.encoder(shape.transpose(1, 2)) #torch.Size([8, 512])
    ...
    def validation_step(self, batch, batch_idx):
        ...
        # features = self.encoder(inputs.transpose(1, 2))
        # sketch_feat, shape_feat = torch.split(features, [B, B], dim=0)
        sketch_feat, matrix = self.encoder(source_shape.transpose(1, 2), matrix=True) #torch.Size([8, 512])
        shape_feat = self.encoder(target_shape.transpose(1, 2)) #torch.Size([8, 512])


        return dict(sketch_feat=sketch_feat, shape_feat=shape_feat, matrix= matrix)
    ...
    #         matrix = []
    #         for output in outputs:
    #             sketch_feat.append(output['sketch_feat'])
    #             shape_feat.append(output['shape_feat'])
    #             matrix.append(output['matrix'])
    #         shape_feat = torch.cat(shape_feat, 0)
    #         sketch_feat = torch.cat(sketch_feat, 0)
    #         matrix = torch.cat(matrix, 0)
    #         # d_feat_z = compute_distance(sketch_features, shape_features, l2=True)
    #         d_feat_z = egocentric(sketch_feat, shape_feat, matrix)
    ...
    #         for k in [1, 5]:
    #             self.log(f'val/CD_d@{k}', top_fitting_gap[:, :k].mean() * 100, prog_bar=True)
    #         if self.hparams.val_metric == 'CD_d':
    #             self.log('val_loss', top_fitting_gap[:, :1].mean() * 100, prog_bar=True)
    #         else:
    #             self.log('val_acc', acc_at_k_feat_z[0], prog_bar=True)
    ...
    def validation_epoch_end(self, outputs):
        if not self.trainer.running_sanity_check:
            ...
            # d_feat_z = compute_distance(sketch_features, shape_features, l2=True)
            d_feat_z = egocentric(sketch_features, shape_features, matrix)
