...
class Deformer_Retrieval(pl.LightningModule):
    ...
    def get_shape_loader(self):
        ...
        val_shape_dataset = SketchyVR_single_multifold(datamodule, type='shape', fold=self.hparams.fold_id)
        ...
        test_shape_dataset = SketchyVR_single(datamodule, mode='test', type='shape')
        ...
        val_list_path = '/vol/vssp/datasets/multiview/3VS/datasets/FineGrained_3DSketch/list/multifold/fold_{}_val.txt'.format(self.hparams.fold_id)
        val_list = [line.rstrip() for line in open(val_list_path)]
        name_index_map = {item: index for index, item in enumerate(val_shape_dataset.name_list)}
        index_list = [name_index_map[item] for item in val_list]
        fitting_gap_path = '/vol/vssp/datasets/multiview/3VS/datasets/cache/multifold/{}.npy'
        self.val_fitting_gap = {item: torch.tensor(np.load(fitting_gap_path.format(item))[index_list, :]).cuda() for item in ['CD_d', 'bi_CD_d']}
    ...
    def evaluate(self, dis_mat):
        ...
        # file_path = '/vol/vssp/datasets/multiview/3VS/datasets/cache/test_selected_metrics/{}.npy'
        file_path = '/vol/research/sketching/projects/VR_Sketch_lightning/project/data/cache/test_selected_metrics/{}.npy'
