...
class Shapes(torch.utils.data.Dataset): 
    ...
    def _get_sketch_path(self, name):
        return os.path.join(self.opt['data_dir'], 'pointcloud', self.opt['sketch_dir'], name + POINT_CLOUD_FILE_EXT)
    ...
    def _get_mesh_path(self, name):
        return os.path.join(self.opt['data_dir'], 'pointcloud', 'shape', name + POINT_CLOUD_FILE_EXT)
